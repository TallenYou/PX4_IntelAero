/**
 * @file df_bmm150_wrapper.cpp
 * Lightweight driver to access the BMM150 of the DriverFramework.
 *
...
#include <bmm150/BMM150.hpp>
#include <DevMgr.hpp>


extern "C" { __EXPORT int df_bmm150_wrapper_main(int argc, char *argv[]); }

using namespace DriverFramework;


class DfBmm150Wrapper : public BMM150
{
public:
    DfBmm150Wrapper(enum Rotation rotation);
    ~DfBmm150Wrapper();
...
DfBmm150Wrapper::DfBmm150Wrapper(enum Rotation rotation) :
    BMM150(MAG_DEVICE_PATH),
    _mag_topic(nullptr),
    _param_update_sub(-1),
    _mag_calibration{},
...
DfBmm150Wrapper::~DfBmm150Wrapper()
{
    perf_free(_mag_sample_perf);
}

int DfBmm150Wrapper::start()
{
    ...
    if (ret != 0) {
        PX4_ERR("BMM150 init fail: %d", ret);
        return ret;
    }
    ...
    if (ret != 0) {
        PX4_ERR("BMM150 start fail: %d", ret);
        return ret;
    }
...
int DfBmm150Wrapper::stop()
{
    /* Stop sensor. */
    int ret = BMM150::stop();

    if (ret != 0) {
        PX4_ERR("BMM150 stop fail: %d", ret);
        return ret;
    }
...
void DfBmm150Wrapper::_update_mag_calibration()
{
...
int DfBmm150Wrapper::_publish(struct mag_sensor_data &data)
{
...
namespace df_bmm150_wrapper
{

DfBmm150Wrapper *g_dev = nullptr;
...
int start(enum Rotation rotation)
{
    g_dev = new DfBmm150Wrapper(rotation);

    if (g_dev == nullptr) {
        PX4_ERR("failed instantiating DfBmm150Wrapper object");
        return -1;
    }
    ...
    if (ret != 0) {
        PX4_ERR("DfBmm150Wrapper start failed");
        return ret;
    }
...
usage()
{
    PX4_INFO("Usage: df_bmm150_wrapper 'start', 'info', 'stop'");
    PX4_INFO("options:");
    PX4_INFO("    -R rotation");
}
...
int
df_bmm150_wrapper_main(int argc, char *argv[])
{
    int ch;
    ...
    if (!strcmp(verb, "start")) {
        ret = df_bmm150_wrapper::start(rotation);
    }

    else if (!strcmp(verb, "stop")) {
        ret = df_bmm150_wrapper::stop();
    }

    else if (!strcmp(verb, "info")) {
        ret = df_bmm150_wrapper::info();
    }
