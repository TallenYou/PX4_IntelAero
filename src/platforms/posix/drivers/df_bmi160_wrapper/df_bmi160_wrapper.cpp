/**
 * @file df_bmi160_wrapper.cpp
 * Lightweight driver to access the BMI160 of the DriverFramework.
 *
 * @author Tallen  <515878013@qq.ch>
 */
...
#define BMI160_NEVER_AUTOPUBLISH_US 0

extern "C" { __EXPORT int df_bmi160_wrapper_main(int argc, char *argv[]); }
using namespace DriverFramework;


class DfBmi160Wrapper : public BMI160
{
public:
    DfBmi160Wrapper(enum Rotation rotation);
    ~DfBmi160Wrapper();
...
DfBmi160Wrapper::DfBmi160Wrapper(enum Rotation rotation) :
    BMI160(IMU_DEVICE_ACC_GYRO),
    ...
    _accel_int(BMI160_NEVER_AUTOPUBLISH_US, false),
    _gyro_int(BMI160_NEVER_AUTOPUBLISH_US, true),
    _publish_count(0),
    _read_counter(perf_alloc(PC_COUNT, "bmi160_reads")),
    _error_counter(perf_alloc(PC_COUNT, "bmi160_errors")),
    _fifo_overflow_counter(perf_alloc(PC_COUNT, "bmi160_fifo_overflows")),
    _fifo_corruption_counter(perf_alloc(PC_COUNT, "bmi160_fifo_corruptions")),
    _gyro_range_hit_counter(perf_alloc(PC_COUNT, "bmi160_gyro_range_hits")),
    _accel_range_hit_counter(perf_alloc(PC_COUNT, "bmi160_accel_range_hits")),
    _publish_perf(perf_alloc(PC_ELAPSED, "bmi160_publish")),
...
DfBmi160Wrapper::~DfBmi160Wrapper()
{
...
int DfBmi160Wrapper::start()
{
    ...
    if (ret != 0) {
        PX4_ERR("BMI160 init fail: %d", ret);
        return ret;
    }
    ...
    if (ret != 0) {
        PX4_ERR("BMI160 start fail: %d", ret);
        return ret;
    }

    PX4_DEBUG("BMI160 device id is: %d", m_id.dev_id);
...
int DfBmi160Wrapper::stop()
{
    /* Stop sensor. */
    int ret = BMI160::stop();

    if (ret != 0) {
        PX4_ERR("BMI160 stop fail: %d", ret);
        return ret;
    }
...
void DfBmi160Wrapper::_update_gyro_calibration()
{
...
void DfBmi160Wrapper::_update_accel_calibration()
{
    // TODO: replace magic number
    for (unsigned i = 0; i < 3; ++i) {
...
int DfBmi160Wrapper::_publish(struct imu_sensor_data &data)
{
...
namespace df_bmi160_wrapper
{

DfBmi160Wrapper *g_dev = nullptr;
...
int start(enum Rotation rotation)
{
    g_dev = new DfBmi160Wrapper(rotation);

    if (g_dev == nullptr) {
        PX4_ERR("failed instantiating DfBmi160Wrapper object");
        return -1;
    }
    ...
    if (ret != 0) {
        PX4_ERR("DfBmi160Wrapper start failed");
        return ret;
    }
...
usage()
{
    PX4_INFO("Usage: df_bmi160_wrapper 'start', 'info', 'stop'");
    PX4_INFO("options:");
    PX4_INFO("    -R rotation");
}
...
int
df_bmi160_wrapper_main(int argc, char *argv[])
{
    int ch;
    ...
    if (!strcmp(verb, "start")) {
        ret = df_bmi160_wrapper::start(rotation);
    }

    else if (!strcmp(verb, "stop")) {
        ret = df_bmi160_wrapper::stop();
    }

    else if (!strcmp(verb, "info")) {
        ret = df_bmi160_wrapper::info();
    }
